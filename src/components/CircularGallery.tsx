...
class Media {
    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          ...
          // Mix with white background if the image contains alpha transparency
          vec3 finalColor = mix(vec3(1.0), color.rgb, color.a);
  ...
  update(scroll: any, direction: any) {
    ...
    // Use a small buffer to ensure wrapping happens completely off-screen
    const boundary = viewportOffset + planeOffset + 0.5;

    if (this.plane.position.x < -boundary) {
      this.extra -= this.widthTotal;
    } else if (this.plane.position.x > boundary) {
      this.extra += this.widthTotal;
    }
  }
