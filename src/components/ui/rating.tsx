  ...
  const icons = React.useMemo(() => {
    const emptyIcon = React.cloneElement(Icon, {
      ...
      className: cn(
        ...
        variant === "yellow" && "fill-amber-600/25 stroke-amber-600/10 text-amber-600/10"
      ),
      "aria-hidden": "true",
    });
...
function Rating({
    ...
    (event: React.KeyboardEvent<HTMLDivElement>) => {
      ...
      switch (event.key) {
        ...
        case "ArrowUp":
          ...
          handleValueChange(value + precision > max ? 0 : value + precision);
          break;
        case "ArrowLeft":
        case "ArrowDown":
          event.preventDefault();
          handleValueChange(value - precision < 0 ? max : value - precision);
          break;
        case " ":
        case "Enter":
          event.preventDefault();
          handleValueChange(value === 0 ? precision : 0);
          break;
        case "Home":
          event.preventDefault();
  ...
  return (
    <div
      ...
      data-readonly={readOnly}
      className={cn(
        "flex gap-px focus-visible:ring-2 focus-visible:ring-brand-red/40 focus-visible:ring-offset-2 focus-visible:outline-none",
        disabled && "opacity-50",
        className
      )}
    ...
    >
      {stars.map(({ key, points }) => (
        <span
          ...
          className={cn("relative", isInteractive && "transition-transform hover:scale-110", disabled && "cursor-not-allowed")}
