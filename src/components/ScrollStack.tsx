...
const ScrollStack = ({
  ...
  // Measures positions once and caches them to avoid getBoundingClientRect layout thrashing during scroll
  const measureTops = useCallback(() => {
    ...
    // Cache original transforms, clear them, measure real offsets, then restore
    ...
    let endElementTop = 0;
    if (endElement) {
      let el: HTMLElement | null = endElement as HTMLElement;
      if (useWindowScroll) {
        while (el) {
    ...
    endElementTopRef.current = endElementTop;
  ...
  useLayoutEffect(() => {
    ...
    // Re-measure positions on window resize to ensure correct values
