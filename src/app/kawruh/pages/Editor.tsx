...
export default function PagesEditor({ editId }: { editId?: number }) {
  ...
  const fetchPostAndCats = useCallback(async () => {
    try {
      if (editId) {
        const res = await fetch(`/api/cms/pages/${editId}`);
        ...
        if (res.ok && data.page) {
          ...
          setIsMetaTitleDirty(!!page.metaTitle);
        } else {
          setError(data.error || "Gagal memuat detail halaman.");
        }
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (editId) {
      document.title = `Edit Halaman: ${title || "Loading..."} | Kawruh Admin`;
    } else {
      document.title = "Buat Halaman Baru | Kawruh Admin";
    }
  }, [editId, title]);
  ...
  const goBack = () => {
    router.push("/kawruh/pages");
  };

  const handleSubmit = async (asDraft?: boolean) => {
    if (!title || !content) { setError("Judul dan konten wajib diisi."); return; }
    setSaving(true); setError("");
    const shouldPublish = asDraft === true ? false : published;
    const url = editId ? `/api/cms/pages/${editId}` : "/api/cms/pages";
    const method = editId ? "PUT" : "POST";
    try {
      const res = await fetch(url, {
        method, headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ title, slug, content, published: shouldPublish, metaTitle, metaDescription }),
      });
      const data = await res.json();
      if (res.ok) {
        toast.success(editId ? "Halaman berhasil diperbarui!" : "Halaman baru berhasil dibuat!");
        if (!editId && data.page?.id) {
          router.push(`/kawruh/pages/edit/${data.page.id}`);
        }
      } else setError(data.error || "Gagal menyimpan halaman.");
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        toast.success("Konten berhasil disimpan untuk pratinjau!");
        let activeId = editId || data.page?.id;
        if (activeId) {
          if (!shouldPublish) {
            window.open(`/?preview-${activeId}`, "_blank");
          } else {
            window.open("/", "_blank");
          }
        }
  ...
  const handleLinkSubmit = () => {
    ...
    if (anchor) {
      ...
      // If text didn't change, just update attributes directly on anchor
  ...
  const quillModules = useMemo(() => ({
    toolbar: {
      container: "#custom-toolbar-pages",
      handlers: {
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center space-y-2">
          ...
          <p className="text-sm font-bold text-gray-900">Memuat Editor Halaman...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center space-x-3">
            <button onClick={goBack} className="text-gray-400 hover:text-gray-600 cursor-pointer"><Icon icon="lucide:arrow-left" className="h-5 w-5" /></button>
            <h1 className="text-xl font-medium text-gray-900 tracking-tight">{editId ? "Edit Halaman" : "Buat Halaman Baru"}</h1>
          </div>
          <p className="text-xs font-normal text-gray-400 mt-1 ml-8">Kelola konten halaman statis Anda.</p>
        </div>
        <div className="flex items-center space-x-2">
          <button type="button" onClick={handlePreview} disabled={saving} className="flex items-center space-x-1.5 px-4 py-2.5 border border-gray-200 bg-white hover:bg-gray-50 text-gray-600 rounded-lg text-xs font-bold cursor-pointer transition-all disabled:opacity-50">
            <Icon icon="lucide:eye" className="h-3.5 w-3.5" /><span>Pratinjau</span>
          </button>
          <button type="button" onClick={() => handleSubmit()} disabled={saving} className="flex items-center space-x-1.5 px-5 py-2.5 bg-brand-red hover:bg-brand-dark text-white rounded-lg text-xs font-bold cursor-pointer transition-all disabled:opacity-50">
      ...
      <div className="flex gap-5 items-start">
        ...
        <div className="flex-1 min-w-0 space-y-4">
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 space-y-4">
            <div>
              <label className="block text-[11px] font-bold text-gray-500 uppercase mb-1">Judul Halaman</label>
            ...
            <div>
              ...
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2"><Icon icon="lucide:link" className="h-3.5 w-3.5 text-gray-400" /></span>
                <input type="text" value={slug} onChange={(e) => setSlug(e.target.value)} placeholder="tentang-kami" className="bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-lg focus:ring-brand-red focus:border-brand-red block w-full p-2.5 pl-9" />
              </div>
              {(title || slug) && (
                <p className="text-[10px] text-gray-400 font-medium mt-1">
                  URL: jakartalanyard.com/<span className="text-gray-500">{autoSlug || "slug-halaman"}</span>
                </p>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex flex-col">
            ...
            {!isHtmlMode && (
              <div id="custom-toolbar-pages" className="!border-none !bg-gray-50/50 p-2 flex flex-wrap items-center">
            ...
            <div className="border-t border-gray-100">
              {isHtmlMode ? (
                <textarea
                  ...
                  className="w-full min-h-[500px] p-5 font-mono text-xs text-gray-800 bg-gray-50 border-none focus:outline-none block resize-y"
          ...
          <SidebarSection title="Publikasikan" icon="lucide:send">
            <div>
              ...
              <div className="flex rounded-lg border border-gray-200 overflow-hidden">
                <button type="button" onClick={() => setPublished(false)} className={`flex-1 flex items-center justify-center space-x-1.5 py-2 text-xs font-bold cursor-pointer transition-all ${!published ? "bg-brand-light-50 text-brand-red" : "bg-white text-gray-500 hover:bg-gray-50"}`}>
                  <Icon icon="lucide:file-text" className="h-3.5 w-3.5" /><span>Draft</span>
                </button>
                <button type="button" onClick={() => setPublished(true)} className={`flex-1 flex items-center justify-center space-x-1.5 py-2 text-xs font-bold cursor-pointer transition-all border-l border-gray-200 ${published ? "bg-brand-light-50 text-brand-red" : "bg-white text-gray-500 hover:bg-gray-50"}`}>
                  <Icon icon="lucide:globe" className="h-3.5 w-3.5" /><span>Terbitkan</span>
                </button>
              </div>
            </div>
          </SidebarSection>

          <SidebarSection title="SEO" icon="lucide:search">
            <div>
            ...
            <div>
              ...
              <div className="relative">
                <textarea rows={2} value={metaDescription} onChange={(e) => setMetaDescription(e.target.value)} placeholder="Masukkan meta description..." maxLength={160} className="bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-lg focus:ring-brand-red focus:border-brand-red block w-full p-2.5" />
            ...
            <div>
              ...
              <div className="bg-gray-50 rounded-lg border border-gray-200 p-3 space-y-1">
                <p className="text-[10px] text-green-700 font-medium truncate">jakartalanyard.com &rsaquo; {autoSlug || "slug-halaman"}</p>
