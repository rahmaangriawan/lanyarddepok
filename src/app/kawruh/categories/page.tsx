...
export default function CategoriesPage() {
  ...
  useEffect(() => {
    document.title = "Kategori Blog | Kawruh Admin";
    fetchCategories();
  }, []);
  ...
  const blogCategories = useMemo(() => {
    return categories.filter((c) => c.type === "BLOG");
  }, [categories]);
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center space-y-2">
          ...
          <p className="text-sm font-bold text-gray-950">Memuat Kategori...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-medium text-gray-900 tracking-tight">Kategori Blog</h1>
        <p className="text-xs font-normal text-gray-400 mt-1">
          Kelola kategori untuk pengorganisasian postingan blog Anda.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white rounded-lg border border-gray-200 shadow-xs overflow-hidden h-fit">
          <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
            <h3 className="text-base font-bold text-gray-900">Daftar Kategori Blog</h3>
          </div>
          {blogCategories.length === 0 ? (
            <div className="p-8 text-center space-y-2">
              <Icon icon="lucide:tag" className="h-10 w-10 text-gray-300 mx-auto" />
              <h4 className="text-sm font-bold text-gray-900">Belum ada kategori blog</h4>
