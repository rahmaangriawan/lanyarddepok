 ...
 */
export function injectAutoLinks(htmlContent: string, links: LinkMapping[], maxLimit = 2): string {
    ...
    if (i % 2 === 1) {
      ...
      if (!inLink && parts[i].trim().length > 0) {
        ...
        for (const item of links) {
          ...
          while ((match = regex.exec(text)) !== null && replacedCount < maxLimit) {
 ...
 */
export function parseFaqs(htmlContent: string): { question: string; answer: string }[] {
  if (!htmlContent) return [];
  const faqs: { question: string; answer: string }[] = [];

  // Match headings (h2/h3/h4) containing "?" followed by a paragraph
  const regex = /<(h[234])[^>]*>([^<]*\?[^<]*)<\/h[234]>\s*<p[^>]*>([\s\S]*?)<\/p>/gi;
  let match;
 ...
 */
export function injectRelatedReading(htmlContent: string, relatedPosts: { title: string; slug: string }[]): string {
  if (!htmlContent || !relatedPosts || relatedPosts.length === 0) return htmlContent;
  ...
  for (let i = 0; i < paragraphs.length; i++) {
    ...
    if (isParagraph2 && insertedCount < 1 && insertedCount < relatedPosts.length) {
...
export function injectTableOfContents(htmlContent: string): string {
  ...
  const headingsToUpdate: { tag: string; originalAttrs: string; originalContent: string; id: string }[] = [];
  ...
  while ((match = headingRegex.exec(htmlContent)) !== null) {
    ...
    const displayDocText = cleanText.replace(/^(?:\d+\.|[A-Za-z]\.)\s*/, "").trim();
  ...
  let headingIndex = 0;
  const htmlWithIds = htmlContent.replace(/<(h[23])([^>]*)>([\s\S]*?)<\/h[23]>/gi, (match, tag, attrs, content) => {
  ...
  const renderH3Subheadings = (subheadings: { id: string; text: string }[]): string => {
  ...
  let tocHtml = `
<div class="toc-container my-8 p-6 sm:p-7 bg-white border border-gray-100 rounded-3xl shadow-xs max-w-2xl select-none">
...
`;
  ...
  if (firstH2Match && firstH2Match.index !== undefined) {
    ...
    return htmlWithIds.slice(0, insertIndex) + tocHtml + htmlWithIds.slice(insertIndex);
  }
